...
log_file = f"{folder_name}/Hybrid_{timestamp}_{num_servers}_{iteration}.log"
logging.basicConfig(
    filename=log_file,
    level=logging.INFO,
...
# Load model
# ----------------------------
def load_model(path: str, input_dims=3, n_actions=1):
    try:
...
class DownloadState:
    def __init__(self):
        ...
        self.last_fast_chunk_mb = 40.0
        self.last_fast_time = 1.0
    ...
    async def set_fast_chunk_info(self, chunk_mb, download_time):
        """Called by fast server to share chunk info"""
        async with self.lock:
            self.last_fast_chunk_mb = chunk_mb
            self.last_fast_time = download_time
    
    async def get_fast_chunk_info(self):
        """Called by slow servers to get fast chunk info"""
        async with self.lock:
            return self.last_fast_chunk_mb, self.last_fast_time
...
# ============================================================================
# HYBRID APPROACH: RL for fast server, proportional for slow servers
# ============================================================================

MODEL_PATH = "/home/ubuntu/MyClient/Models/Model_Hybrid.pth"

def predict_fast_chunk_mb(model, server_speeds_mb: np.ndarray) -> float:
    """Predict chunk size for fastest server using RL"""
    if model is None:
        return 40.0
    ...
    try:
        with torch.no_grad():
            ...
            chunk_mb = ACTION_MIN_MB + y[0].item() * (ACTION_MAX_MB - ACTION_MIN_MB)
            return chunk_mb
    except Exception as e:
        logging.error(f"Prediction error: {e}")
        return 40.0

async def download_file_hybrid(session, url, state: DownloadState, model, server_idx):
    """Each server runs independently, continuously grabbing chunks"""
    ...
    # Continuous loop - no waiting for other servers
    while True:
        ...
        if np.all(speeds_bps > 0):
            # Am I the fastest?
            fastest_idx = np.argmax(speeds_bps)
            fastest_speed_bps = speeds_bps[fastest_idx]
            is_fastest = (server_idx == fastest_idx)
            
            if is_fastest:
                # RL prediction for fast server (matches old code)
                large_chunk_mb = predict_fast_chunk_mb(model, speeds_mbps)
                large_chunk_bytes = int(large_chunk_mb * 1024 * 1024)
                
                # Calculate expected download time
                fast_download_time = large_chunk_bytes / max(fastest_speed_bps, 1e-9)
                
                # Share this info with slow servers
                await state.set_fast_chunk_info(large_chunk_mb, fast_download_time)
                
                chunk_bytes = large_chunk_bytes
                
                logging.info(f"[SERVER {server_idx}] Fast server, RL predicted: {large_chunk_mb:.1f} MB, "
                           f"expected time: {fast_download_time:.2f}s")
            else:
                # Slow server: get fast server's info
                fast_chunk_mb, fast_download_time = await state.get_fast_chunk_info()
                
                # Calculate proportional chunk (EXACTLY matches old code)
                my_speed_bps = speeds_bps[server_idx]
                small_chunk_bytes = math.floor(fast_download_time * my_speed_bps)
                
                chunk_bytes = small_chunk_bytes
                
                logging.info(f"[SERVER {server_idx}] Slow server, proportional: {chunk_bytes/(1024**2):.1f} MB "
                           f"(fast_time={fast_download_time:.2f}s, my_speed={my_speed_bps/(1024**2):.2f} MB/s)")
        else:
...
async def main():
    logging.info("=== HYBRID APPROACH START ===")
    
    model = load_model(MODEL_PATH, input_dims=3, n_actions=1)
    ...
    try:
        ...
        worker_results = await asyncio.gather(
            *(download_file_hybrid(sessions[url], url, state, model, i) 
              for i, url in enumerate(file_sources)),
        ...
        '''
        # Write file
        filename = f"{folder_name}/output_hybrid_{iteration}"
        ...
        logging.info("=== HYBRID APPROACH END ===")
        
        print(f"✓ Hybrid complete: {total_time:.2f}s, {total_chunks} chunks, "
