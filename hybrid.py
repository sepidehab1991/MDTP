...
# ----------------------------
MODEL_PATH = "/home/ubuntu/DRL/Models/hybrid_Markov_April.pth"
C_MIN_MB = 10.0
C_MAX_MB = 128.0
...
# Load trained model
# ----------------------------
def load_trained_model(path: str, input_dims=13, n_actions=1):
...
# ----------------------------
def predict_large_chunk(model, state_vector: np.ndarray) -> float:
    """
    Predict large chunk size in MB using the trained model.

    Args:
        model: Trained ActorCritic model
        state_vector: np.ndarray of shape [13]
                      [6 bandwidths | 6 transfer rates | 1 remaining file size]  (all in MBps / MB)
    ...
    try:
        with torch.no_grad():
            obs = torch.tensor(state_vector, dtype=torch.float32).unsqueeze(0)  # [1, 13]
...
urls = [
    "http://10.133.1.2/", "http://10.141.1.2/", "http://10.135.132.2/",
    "http://10.132.132.2/", "http://10.136.130.2/", "http://10.135.140.2/"
]
...
async def download_file(session, url, throughput, range_dict, model):
    ...
    initial_chunk = 5 * 1024 * 1024  # 5 MB probe
    ...
    while end_byte < file_size and start_byte < file_size:
        ...
        if throughput.get(url, 0.0) == fastest_server:
            ...
            # Build 13-dimensional state vector: [6 bandwidths | 6 transfer rates | 1 remaining]
            bandwidths = np.array([
                throughput.get(file_sources[i], 0.0) if i < len(file_sources) else 0.0
                for i in range(6)
            ], dtype=np.float32)
            ...
            remaining = np.array([file_size - start_byte], dtype=np.float32)

            # Convert to MBps / MB and concatenate into 13-dim state
            state_vector = np.concatenate([bandwidths, transfer_rates, remaining]) * INV_MIB
            logging.info(f"State vector (MBps): {state_vector}")

            # Predict large chunk size using the model
            large_chunk_mb = predict_large_chunk(model, state_vector)
...
async def main():
    # Load trained model (input_dims=13: 6 bandwidths + 6 transfer rates + 1 remaining)
    model = load_trained_model(MODEL_PATH, input_dims=13, n_actions=1)
